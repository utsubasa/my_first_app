...
def process_files(test_x):  
    #データ読み込み
    #x_df_read = pd.read_csv('train_x.csv', sep=',')
    #y_df = pd.read_csv('train_y.csv', sep=',')
    x_test_df = pd.read_csv(test_x, sep=',')


    #予測
    rfr = pickle.load(open("/Users/u_tsubasa/work/play/djangogirls/Regression/rfr_model.sav", 'rb'))
    #list_row = pickle.load(open("/Users/u_tsubasa/work/play/djangogirls/Regression/list.txt","rb"))
    #print(list_row)
    #y_pred = rfr.predict(X_valid)
    #y_pred_test = rfr.predict(x_test_df[list_row])
...
def csvdownload(ans_list):
    ...
    for course in courses:
        remaining = availability_dict[course]
